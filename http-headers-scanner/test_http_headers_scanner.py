...
def test_grade_threshold_a_at_90_percent() -> None:
    ...
    “Passing every high and every medium rule should produce an A”
    Two `high` rules = 60 / 100 = 60%, which is grade D
    “Lows may be missing, but the weighted score remains >= 90”
    """
...
def test_grade_threshold_b_with_three_mediums() -> None:
    """
    Two highs ok, two mediums ok, one medium weak, and lows missing
    should land in the B range under the expanded rules table.
    """
    # Two highs ok, two mediums ok, one medium weak, lows missing
    statuses: list[Status] = []
    medium_seen = 0
    for rule in RULES:
        if rule.severity == "high":
            ...
            statuses.append("ok" if medium_seen < 2 else "weak")
            medium_seen += 1
        else:
