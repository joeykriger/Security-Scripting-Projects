...
HASHCAT_MODES = {
    ...
    "SHA-512": 1700,
    "bcrypt": 3200,
    "Argon2id": 3200,  # bcrypt and Argon2id share the same hashcat mode
}
...
def _is_hex_with_0x_prefix(text: str) -> bool:
    if not text.startswith("0x"):
        return False 
    return _is_hex(text[2:])

_BASE58_CHARSET = frozenset(
    "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"
)

def _is_base58(text: str) -> bool:
    return bool(text) and all(c in _BASE58_CHARSET for c in text)
    
_BASE32_CHARSET = frozenset("ABCDEFGHIJKLMNOPQRSTUVWXYZ234567")

def _is_base32(text: str) -> bool:
    return bool(text) and all(c in _BASE32_CHARSET for c in text)

def split_record(raw_input: str) -> list[str]:
    return raw_input.split(":")

def identify_record(raw_input: str) -> list[tuple[str, list[HashCandidate]]]:
    fields = split_record(raw_input)
    return [(field, identify(field)) for field in fields]

def classify_field(field: str, candidates: list[HashCandidate]) -> str:
    if not field:
        return "empty"
    if field.isalpha() and len(field) <= 20:
        return "username"
...
def identify(raw_input: str) -> list[HashCandidate]:
    ...
    if _is_hex_with_0x_prefix(text):
        return [
            HashCandidate(
...
def _build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--split", action="store_true", help="Split colon-separated records and identify each field",
    )
    return parser
...
def _render_split_table(record, console):
        table = Table(title="Split record")
        table.add_column("Field #", style="bold")
        table.add_column("Text")
        table.add_column("Kind")
        table.add_column("Top guess")
...
def main() -> int:
    ...
    hashes_to_process = []
    if args.hash:
        hashes_to_process.append(args.hash)
    if args.file:
        hashes_to_process.extend([line.strip() for line in args.file if line.strip()])
