...
class FrequencyAnalyzer:
    ...
    def get_letter_frequencies(self, text: str) -> dict[str, int]:  # CHALLENGE 3
        text_upper = text.upper()
        letters = [char for char in text_upper if char.isalpha()]
        return dict(Counter(letters))
        
        sorted_frequencies = sorted(  # CHALLENGE 3
            frequencies.items(),
            key = lambda item: (-item[1], item[0])
        )

        for letter, count in sorted_frequencies:  # CHALLENGE 3
            bar = " " * count
